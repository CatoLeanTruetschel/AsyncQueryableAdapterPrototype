...
{
    internal sealed class NullParameterTestCase : TestCase
    {
        ...
        protected override string Name => $"{_uniqueIdentifier}Null{Parameter.Name.FirstRuneToUpperCase()}ThrowsArgumentNullException";
        ...
        {
            await writer.WriteLineAsync($"            await Assert.ThrowsAsync<ArgumentNullException>(async () =>").ConfigureAwait(false);
            await writer.WriteLineAsync($"            {{").ConfigureAwait(false);
