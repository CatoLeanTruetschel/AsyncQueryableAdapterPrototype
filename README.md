...
To implement support for a database driver, create a class that inherits from `AsyncQueryableAdapter.QueryAdapterBase` and implement at least the methods `IQueryable<T> GetQueryable<T>()` and `EvaluateAsync<T>`. For better performance, implement any of the virtual methods that the database driver has native support for. For the virtual methods, where there is no override, a specialized polyfill is used.
...
Please not that we assume here the the collection name is the same as the typename and that the implementation of the `EvaluateAsync<T>` method is not very optimized.
